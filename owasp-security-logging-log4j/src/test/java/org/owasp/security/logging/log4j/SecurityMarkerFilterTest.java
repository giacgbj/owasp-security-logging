...
public class SecurityMarkerFilterTest {
    ...
    private static final org.slf4j.Logger LOGGER = LoggerFactory.getLogger(SecurityMarkerFilterTest.class);
    ...
    public void getMarkersTest() {
        Marker test1 = SecurityMarkers.getMarker(SecurityMarkers.SECURITY_AUDIT);
    ...
    public void testRaw() {
        ...
        LOGGER.info(SecurityMarkers.getMarker(SecurityMarkers.CONFIDENTIAL, SecurityMarkers.RESTRICTED), "This statement is confidential and restricted");
    ...
    /*
     * @Test public void testRawAcceptAll() { //create a new marker filter SecurityMarkerFilter mkt = new SecurityMarkerFilter(); mkt.setContext(loggerContext); mkt.setAcceptAll("true"); mkt.start();
     * 
     * assertTrue(mkt.isStarted());
     * 
     * //test a logging event with no markers ILoggingEvent nulEvent = new LoggingEvent(); assertEquals(FilterReply.DENY, mkt.decide(nulEvent));
     * 
     * //test a logging event with the CONFIDENTIAL marker LoggingEvent confidentialEvent = new LoggingEvent(); confidentialEvent.setMarker(SecurityMarkers.SECURITY_SUCCESS);
     * assertEquals(FilterReply.ACCEPT, mkt.decide(confidentialEvent));
     * 
     * //test a logging event with the RESTRICTED marker LoggingEvent restrictedEvent = new LoggingEvent(); restrictedEvent.setMarker(SecurityMarkers.SECURITY_FAILURE);
     * assertEquals(FilterReply.ACCEPT, mkt.decide(restrictedEvent));
     * 
     * //test a logging event with the SECRET marker LoggingEvent secretEvent = new LoggingEvent(); secretEvent.setMarker(SecurityMarkers.SECURITY_AUDIT); assertEquals(FilterReply.ACCEPT,
     * mkt.decide(secretEvent)); }
     */
