...
public class DefaultIntervalProperty implements IntervalProperty {
    ...
    /**
     * Signal to update the property value. Must be implemented if the property value changes at runtime. For example, to develop a property to update system memory consider overriding with the
     * following, <code>
     * public void refresh() {
     *   value = Long.toString(Runtime.getRuntime().totalMemory());
     * }
     * </code> Specific refresh behavior depends upon the type of property. As general rule if your data does not change then you should consider logging the data outside the status message.
     */
    @Override
    public void refresh() {
